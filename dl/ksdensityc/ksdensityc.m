function [vfEstimate] = ksdensityc(vfObservations, vfPDFSamples, vfDomain, fSigma, vfWeights)

% ksdensityc FUNCTION - Compute a kernel density estimate over a periodic domain
%
% Usage: [vfEstimate] = ksdensityc(vfObservations, vfPDFSamples, 
...
if (nargin < 2)
   help ksdensityc;
   error('ksdensityc:Usage', ...
         '*** ksdensityc: Incorrect usage');
end
...
elseif (numel(vfObservations) ~= numel(vfWeights))
   error('ksdensityc:Usage', ...
         '*** ksdensityc: The number of observations must be equal to the number of weights.');
end
