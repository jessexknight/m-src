...
markersize = 4;
xdatamode = 2; % use the (1) mean of data1 and data2 or (2) data1 for x position on bland altman
...
%a = axis(dah); % __JK__
if fixylim
	text(double(a(2)),double(1+mn+cr),[num2str(mn+cr,2) ' (+1.96 \sigma)'], 'HorizontalAlignment','right','VerticalAlignment','bottom');
	text(double(a(2)),double(1+mn),   [num2str(mn,   2) ' (\mu)'],          'HorizontalAlignment','right','VerticalAlignment','bottom');
	text(double(a(2)),double(1+mn-cr),[num2str(mn-cr,2) ' (-1.96 \sigma)'], 'HorizontalAlignment','right','VerticalAlignment','bottom');
end
BAtext = {};
